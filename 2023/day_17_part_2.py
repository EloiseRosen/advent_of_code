...
while min_heap:
    ...
    if (r,c, last_r_change,last_c_change, steps_in_dir) not in seen:
        ...
        if (r, c) == END and steps_in_dir >= MIN_STEPS_IN_DIR:
            print(cost)
            break
        ...
        for r_change, c_change in DIRS:
            ...
            if (new_r >= 0 and new_r < len(grid) and new_c >= 0 and new_c < len(grid[0]) and  # in bounds
                ...
                (steps_in_dir < MAX_STEPS_IN_DIR or (last_r_change, last_c_change) != (r_change, c_change)) and # can't move in same dir more than specified times
                (steps_in_dir >= MIN_STEPS_IN_DIR or (r_change, c_change) == (last_r_change, last_c_change) or (last_r_change, last_c_change) == (0, 0))): # need to meet minimum steps in dir
