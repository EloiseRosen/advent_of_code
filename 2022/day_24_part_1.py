...
# (dist, row_idx, col_idx, current blizzard locations). dist is first so that we're popping the vertex with min distance
priority_q = [(0, *START, blizzards)]
done = False
visited = set()
while priority_q and not done:
    ...
    if popped not in visited:
        visited.add(popped)
        curr_dist, curr_row, curr_col, blizzards = popped
        ...
        for row_change, col_change in SEARCH_DIRS:
            ...
            (new_row, new_col, '^') not in next_step_blizzards and (new_row, new_col, 'v') not in next_step_blizzards):
                ...
                if (new_row, new_col) == END:
                    ...
                    break
                heappush(priority_q, (curr_dist + 1, new_row, new_col, next_step_blizzards))
