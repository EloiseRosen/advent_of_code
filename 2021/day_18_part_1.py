...
root_of_result = create_binary_tree(lst[0])
for idx in range(1, len(lst)):
    root_of_new_num = create_binary_tree(lst[idx])
    root_of_result = snailfish_add(root_of_result, root_of_new_num)

    while True:
        # If any pair is nested inside four pairs, the leftmost such pair explodes.
        changed = explode(root_of_result)
        if changed:
            continue

        # If any regular number is 10 or greater, the leftmost such regular number splits.
        changed = split(root_of_result)
        if changed:
            continue
...
print('magnitude', get_magnitude(root_of_result))
